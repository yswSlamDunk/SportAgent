...
class ImageInformationNode(BaseNode):
    ...
    def execute(self, state: GraphState) -> Dict:
        ...
        # 체인 실행
        results = image_information_chain(image_dataset, id_list)
...
class ChartInformationNode(BaseNode):
    ...
    def execute(self, state: GraphState) -> Dict:
        ...
        # 체인 실행
        results = chart_information_chain(chart_dataset, id_list)
...
class TableNormalConvertingNode(BaseNode):
    ...
    def execute(self, state: GraphState) -> Dict:
        ...
        # 체인 실행
        table_json_results = normal_table_conversion_chain(normal_dataset)
        
        # 결과 처리
        normal_table = state['normal_table'].copy()
        for i, table_json in enumerate(table_json_results):
            normal_table[i]['dataset']['table_json'] = table_json
        
        self.log(f"Processing completed: {len(table_json_results)} normal tables converted")
...
class TableCutoffConvertingNode(BaseNode):
    ...
    def execute(self, state: GraphState) -> Dict:
        ...
        # 체인 실행
        table_json_results = cutoff_table_conversion_chain(cutoff_dataset)
        ...
        self.log(f"Processing completed: {len(table_json_results)} cutoff tables converted")
...
class TableInformationNode(BaseNode):
    ...
    def execute(self, state: GraphState) -> Dict:
        ...
        # 테이블이 없는 경우 빈 결과 반환
        if len(normal_dataset) == 0 and len(cutoff_dataset) == 0:
        ...
        normal_results = []
        if normal_dataset:
            normal_results = table_information_chain(normal_dataset, normal_id_list, is_normal=True)
            self.log(f"Processed {len(normal_results)} normal tables")
        
        # 잘린 테이블 처리
        cutoff_results = []
        if cutoff_dataset:
            cutoff_results = table_information_chain(cutoff_dataset, cutoff_id_list, is_normal=False)
...
class AlignNode(BaseNode):
    ...
    def _process_media_element(self, category, element, doc_id, element_id, info_map):
        ...
        if 'raw_result' in element_info:
            ...
            # 정상 파싱된 데이터 사용
            detail = element_info.get('detail', '')
            hypotheticalQuestions = element_info.get('hypotheticalQuestions', [])
